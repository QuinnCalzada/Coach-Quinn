...
def recent_sessions_from_source(days=7):
    if USE_SHEETS:
...
def build_prompt(personality, log):
    athlete = log["athlete"]
    recents = recent_sessions_from_source(days=7)
    recent_text = "\n".join([
...
Task:
1) Generate TODAY's training plan with exact sets, reps, stroke rates, HR targets, and estimated target splits relative to 2k pace when useful. Include warm-up and cool-down.
2) Detect recovery risk from recent HR/RPE. If high, adjust plan. Do not glorify overtraining.
3) Provide a short 'Coach Quinn' motivation (1–2 lines).
4) Provide 1–2 session focus cues.

Format as Discord-friendly markdown.
"""
    return system, user
